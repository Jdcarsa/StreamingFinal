...
EXECUTE REGISTRAR_ACTUACION(1, 21, 'PRINCIPAL');
EXECUTE REGISTRAR_ACTUACION(201, 21, 'SECUNDARIO');
EXECUTE REGISTRAR_ACTUACION(241, 22, 'PRINCIPAL');
EXECUTE REGISTRAR_ACTUACION(241, 23, 'PRINCIPAL');
EXECUTE REGISTRAR_ACTUACION(261, 22, 'SECUNDARIO');
EXECUTE REGISTRAR_ACTUACION(261, 21, 'SECUNDARIO');
EXECUTE REGISTRAR_ACTUACION(271, 24, 'SECUNDARIO');
EXECUTE REGISTRAR_ACTUACION(241, 25, 'PRINCIPAL');
EXECUTE REGISTRAR_ACTUACION(201, 27, 'SECUNDARIO');
----------------------------------------------------------------------------------
/*
    Procedimientos:
...
BEGIN
    ...
    SELECT PS.NOMBRE, COUNT(*) AS TOTAL_COMPRASROLLBACK TO SAVEPOINT inicio_transaccion;
...
*/
---------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION PELICULAS_PROTAGONIZADAS(P_NOMBRE_ACTOR IN ACTOR.PRIMERNOMBRE%TYPE) 
...
BEGIN
    OPEN V_RESULTADO FOR
        ...
        FROM ACTOR A JOIN ACTOR_PRODUCTO AP 
        ON A.CODIGO = AP.CODIGO_ACTOR
        JOIN PRODUCTO P 
        ON AP.CODIGO_PRODUCTO = P.CODIGO
        WHERE A.PRIMERNOMBRE = P_NOMBRE_ACTOR;
...
--FUNCION QUE ME DEVUELVA EL NOMBRE DELACTOR
CREATE OR REPLACE FUNCTION OBTENER_NOMBRE_ACTOR(CODIGO_ACTOR IN NUMBER) RETURN VARCHAR2 AS
  V_NOMBRE_ACTOR ACTOR.PRIMERNOMBRE%TYPE;
BEGIN
  SELECT PRIMERNOMBRE INTO V_NOMBRE_ACTOR
  FROM ACTOR
  WHERE CODIGO = CODIGO_ACTOR;

  RETURN V_NOMBRE_ACTOR;
END;

CREATE OR REPLACE FUNCTION OBTENER_NOMBRES(NOMBRE IN ACTOR.PRIMERNOMBRE%TYPE)
  RETURN VARCHAR2
  DETERMINISTIC
AS
  V_SEGUNDO ACTOR.SEGUNDONOMBRE%TYPE;
  V_PRIMERAPELLIDO ACTOR.PRIMERAPELLIDO%TYPE;
  V_SEGUNDOAPELLIDO ACTOR.SEGUNDOAPELLIDO%TYPE;
  V_RESULTADO VARCHAR2(80);
BEGIN
SELECT SEGUNDONOMBRE, PRIMERAPELLIDO, SEGUNDOAPELLIDO
INTO V_SEGUNDO, V_PRIMERAPELLIDO, V_SEGUNDOAPELLIDO
FROM ACTOR
WHERE PRIMERNOMBRE = NOMBRE;
  V_RESULTADO := NOMBRE ||' '|| V_SEGUNDO ||' '|| V_PRIMERAPELLIDO ||' '|| V_SEGUNDOAPELLIDO;
  RETURN V_RESULTADO;
END;

CREATE OR REPLACE FUNCTION OBTENER_DESCRIPCION(NOMBRE IN PRODUCTO.NOMBRE%TYPE)
  RETURN VARCHAR2
  DETERMINISTIC
AS
...
CREATE OR REPLACE TRIGGER TRIGGER_EXISTEADMIN
...
BEGIN
    ...
    INTO VCONTADORADMIN 
    FROM ADMINISTRADOR
    WHERE NOMBRE_USUARIO_ADMIN = VUSUARIOA;

    IF VCONTADORADMIN = 1 THEN
...
CREATE OR REPLACE TRIGGER TRIGGER_EXISTEADMIN
    ...
    BEFORE STATEMENT IS
        BEGIN
            ...
            IF GESTIONCUENTAS.VCONTADORADMIN > 0 THEN
                RAISE_APPLICATION_ERROR(-20024 , 'Ya existe un administrador registrado con ese nombre de usuario');
...
END TRIGGER_EXISTEADMIN;
---------------------------------------------------------------

EXECUTE CREAR_ADMINISTRADOR(16,  'Mantenimiento', 'JOSE', 'BRUNO', 'BARRRAGAN' , 'DIAZ', '19/12/1990', 'PANCONQUESO', '18/04/2023', '3135618190', 'TVPLUSS@TVPLUSCOM');
...
--Crear trigger que actualice el usuario de la tarjeta cuando se actualice el usuario del cliente

CREATE OR REPLACE TRIGGER ACTUALIZAR_NOMBRE_USUARIO_CLIENTE
...
--TRIGGER PARA VERIFICAR LA SUSCRIPCION DE UN CLIENTE
CREATE OR REPLACE TRIGGER VERIFICAR_SUSCRIPCION_CLIENTE
INSTEAD OF INSERT ON VISTA_CLIENTE_PLAN
FOR EACH ROW
DECLARE
    V_CODIGO NUMBER := :NEW.CODIGO_CLIENTE;
    V_CANTIDAD_SUSCRIPCIONES NUMBER;
BEGIN
    SELECT COUNT(*) INTO V_CANTIDAD_SUSCRIPCIONES
    FROM CLIENTE_PLAN
    WHERE CODIGO_CLIENTE = V_CODIGO;

    IF V_CANTIDAD_SUSCRIPCIONES > 0 THEN
        RAISE_APPLICATION_ERROR(-20001, 'Ya existe una suscripcion para este cliente.');
    ELSE
        INSERT INTO CLIENTE_PLAN (CODIGO, CODIGO_CLIENTE, CODIGO_PLAN, FECHA_COMPRA, FECHA_VENCIMIENTO)
        VALUES (:NEW.CODIGO, :NEW.CODIGO_CLIENTE, :NEW.CODIGO_PLAN, :NEW.FECHA_COMPRA, :NEW.FECHA_VENCIMIENTO);
    END IF;
END;
