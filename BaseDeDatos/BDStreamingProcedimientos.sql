...
/*
    ...
    PROCEDIMIENTOS DE LA LÓGICA DEL DICCIONARIO DE DATOS
*/
----------------------------------------------------------------------------------

--PROYECTA TODAS LAS TABLAS DEL ESQUEMA CON SU NOMBRE, TIPO DE OBJETO Y FECHA DE CREACIÓN

CREATE OR REPLACE PROCEDURE PROYECTAR_TABLAS(REG_TABLAS OUT SYS_REFCURSOR)
AS
BEGIN
    OPEN REG_TABLAS FOR SELECT OBJECT_NAME, OBJECT_TYPE, CREATED  FROM USER_OBJECTS WHERE OBJECT_TYPE = 'TABLE';
END;

--PROYECTA TODAS LOS PROCEDIMIENTOS DEL ESQUEMA CON SU NOMBRE, TIPO DE OBJETO Y FECHA DE CREACIÓN

CREATE OR REPLACE PROCEDURE PROYECTAR_PROCEDIMIENTOS(REG_PROCEDIMIENTOS OUT SYS_REFCURSOR)
AS
BEGIN
    OPEN REG_PROCEDIMIENTOS FOR SELECT OBJECT_NAME, OBJECT_TYPE, CREATED  FROM USER_OBJECTS WHERE OBJECT_TYPE = 'PROCEDURE';
END;

--PROYECTA TODAS LAS COLUMNAS DE UNA TABLA DEL ESQUEMA CON SU NOMBRE

CREATE OR REPLACE PROCEDURE PROYECTAR_COLUMNAS(V_NOMBRE_TABLA IN VARCHAR2, REG_PROCEDIMIENTOS OUT SYS_REFCURSOR)
AS
BEGIN
    OPEN REG_PROCEDIMIENTOS FOR SELECT column_name, data_type, nullable
    FROM all_tab_columns
    WHERE table_name = V_NOMBRE_TABLA;
END;

--PROYECTA TODAS LAS RESTRICCIONES DE UNA TABLA DEL ESQUEMA CON SU NOMBRE

CREATE OR REPLACE PROCEDURE PROYECTAR_RESTRICCIONES(V_NOMBRE_TABLA IN VARCHAR2, REG_RESTRICCIONES OUT SYS_REFCURSOR)
AS
BEGIN
    OPEN REG_RESTRICCIONES FOR SELECT constraint_name, constraint_type, search_condition
